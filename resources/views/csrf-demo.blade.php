...
<body class="bg-gray-100">
    <div class="max-w-3xl mx-auto mt-10 bg-white shadow-md rounded-lg p-6">
        ...
        @if ($step == 1)
            ...
            <p class="mb-6">Yesterday, you logged into your account on your favorite website—let’s call it "SecureSite." You entered your credentials, 
                and everything worked fine. You went about your day, and now today, you’re still logged in, your session is still active.</p>
        ...
        <!-- @elseif ($step == 3)
            {{-- Scene 3: Clicking the Link --}}
            <h1 class="text-2xl font-bold mb-4">Scene 3: Clicking the Link</h1>
            <p class="mb-6">You clicked the link in the email and were taken to a page that looks exactly like the SecureSite password reset form:</p>
            <form action="/password-reset" method="POST" class="bg-gray-100 p-4 rounded border">
                @csrf
                <label for="password" class="block text-sm font-medium text-gray-700">Enter New Password:</label>
                <input type="password" id="password" name="password" class="mt-2 p-2 w-full border rounded" placeholder="New Password">
                <button type="submit" class="mt-4 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                    Submit
                </button>
            </form>
            <p class="mt-4 text-red-500">What you don’t realize is that this is a fake page designed by an attacker. Your password is being sent to their server!</p>
            <a href="{{ route('csrf.demo', ['step' => 1]) }}" class="mt-6 inline-block bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
                Start Over
            </a> -->
        @elseif ($step == 'phishing')
            {{-- Scene 3b: Phishing Page --}}
            <h1 class="text-2xl font-bold mb-4">Phishing Page</h1>
            ...
            <form action="{{ route('csrf.demo', ['step' => 'phishing-success']) }}" method="POST">
                ...
                <label for="password" class="block text-sm font-medium text-gray-700">Enter New Password:</label>
                <input type="password" id="password" name="password" class="mt-2 p-2 w-full border rounded" placeholder="New Password">
                <label for="password" class="block text-sm font-medium text-gray-700">Confirm Your New Password:</label>
                <input type="password" id="password" name="password" class="mt-2 p-2 w-full border rounded" placeholder="New Password">
                <button type="submit" class="mt-4 bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">
                    Update
                </button>
            </form>
            <!-- <p class="mt-4 text-red-500">This is a phishing page. Be cautious about entering sensitive information on unknown links.</p> -->
            <!-- <a href="{{ route('csrf.demo', ['step' => 2]) }}" class="mt-6 inline-block bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                Go Back
            </a> -->
        <!-- @elseif ($step == 'Update-Details')
            {{-- Scene 3c: Phishing Success --}}
            <h1 class="text-2xl font-bold mb-4">Details Updated Successfully</h1>
